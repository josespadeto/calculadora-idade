const translations = {
    pt: {
        ...
        years: "anos", resultText: "Resultado:", affText: "Dica de Saúde:", affLink: "Ver na Amazon", privacy: "Privacidade", error: "Preencha a idade!"
    },
    en: {
        ...
        years: "years old", resultText: "Result:", affText: "Health Tip:", affLink: "Shop on Amazon", privacy: "Privacy", error: "Enter age!"
    },
    es: {
        ...
        years: "años", resultText: "Resultado:", affText: "Dato Saludable:", affLink: "Ver en Amazon", privacy: "Privacidad", error: "¡Poner edad!"
    }
};
...
function setLanguage(lang) {
    ...
    document.getElementById('lbl-age-bio').innerText = t.lblAge;
...
function calculateCal() {
    ...
    if(!w || !a) { alert(translations[currentLang].error); return; }
    let tmb = (g === 'male') ? (10 * w + 900 - 5 * a) : (10 * w + 700 - 5 * a);
    document.getElementById('result').innerHTML = `${tmb.toFixed(0)} kcal / dia`;
    showAff();
}

function calculateBio() {
    const crono = parseInt(document.getElementById('age-bio').value);
    if(!crono) { alert(translations[currentLang].error); return; }
    const ex = parseInt(document.getElementById('bio-ex').value);
    const dt = parseInt(document.getElementById('bio-diet').value);
    const finalBio = crono + ex + dt;
    document.getElementById('result').innerHTML = `Bio-Idade: ${finalBio} ${translations[currentLang].years}`;
    showAff();
}
